...
# Use a stable per-process salt to reduce cross-run cache-key collisions if ids repeat.
_CACHE_SALT = str(id(object()))
...
async def _filter_nodes(raw_nodes: List[NodeWithScore], qlem: FrozenSet[str]) -> List[NodeWithScore]:
    ...
    # Fallback: require at least 1 overlapping lemma
    fallback: List[Tuple[NodeWithScore, float]] = []
    for n in raw_nodes:
        s = _safe_score(n.score)
        lemmas = _metadata_lemmas(n)
        inter = len(qlem & lemmas)
        ...
        ratio = inter / qlen
        weight = s + alpha * ratio
        fallback.append((n, weight))

    fallback.sort(key=lambda x: x[1], reverse=True)
    return [n for n, _ in fallback[:TOP_K]]
