...
def _require_state() -> AppDeps:
    if _STATE is None:
        raise RuntimeError(
            "Discord module is not initialized. Call run_bot(...) or start_bot_async(...)."
        )
    return _STATE
...
async def _answer_rag(
    ...
    try:
        ...
        await ctx.send("⚠️ System prompt file is missing.")
        return
    except UnicodeError:
        await ctx.send("⚠️ System prompt file is not valid UTF-8.")
        return
    except asyncio.TimeoutError:
        await ctx.send("⚠️ Request timed out. Try again later.")
        return
    except aiohttp.ClientError as exc:
        disc_log.warning("[_answer_rag] network error: %s", exc)
        await ctx.send("⚠️ Network error while contacting the model. Try again later.")
        return
    except ValueError as exc:
        disc_log.info("[_answer_rag] bad request: %s", exc)
        await ctx.send("⚠️ Request rejected by validation.")
        return
    except discord.HTTPException as exc:
        disc_log.warning("[_answer_rag] discord HTTP error: %s", exc)
        await ctx.send("⚠️ Discord API error while sending the response.")
        return
...
def _register_commands(bot: commands.Bot) -> None:
    @bot.command(name="strict", help="Answer using strict prompt and RAG.")
    @_check_channel_allowed()
    async def cmd_strict(ctx: commands.Context, *, q: str) -> None:
        await _answer_rag(ctx, q, prompt_path=Path(cfg.PROMPT_STRICT), force_local=None)

    @bot.command(name="think", help="Answer using reasoning prompt and RAG.")
    @_check_channel_allowed()
    async def cmd_think(ctx: commands.Context, *, q: str) -> None:
        await _answer_rag(ctx, q, prompt_path=Path(cfg.PROMPT_REASON), force_local=None)
    ...
    async def cmd_local(ctx: commands.Context, *, q: str) -> None:
        await _answer_rag(ctx, q, prompt_path=Path(cfg.PROMPT_STRICT), force_local=True)
    ...
    async def cmd_reload_index(ctx: commands.Context) -> None:
        ...
        try:
            ...
            await ctx.send("❌ Internal error while rebuilding index.")
